...
*  IMANAG   number of management data set             -           I        *
...
*  EMERG    indicates crop emergence                  -           O,C      *
...
*  File usage: LINTER                                                      *
*------------------------------------------------------------------------
       SUBROUTINE CROPV(ICROP,INITC,IOPT, IDAY,IDEM,IDEMERG,IDPL,IDFLOW,
	 ...
	 INTEGER ILTMPF,ILTMNF,ILFR,ILFL,ILFS,ILFO,ILRDRL,IMANAG
	 INTEGER ILRDRR,ILRDRS,ILNMXL,ILPHOT,ILFERT,ILNRFT, ILVER 

       LOGICAL TERMIN,INITC,PL,FLOW,EMERG,YCH,CRP
...
*     Read management data from file MANAGE.DAT
      IF (IMANAG .EQ. 1) Call RDINIT (15,0, 'MANAG1.DAT')
      IF (IMANAG .EQ. 2) Call RDINIT (15,0, 'MANAG2.DAT')
      IF (IMANAG .EQ. 3) Call RDINIT (15,0, 'MANAG3.DAT')
      IF (IMANAG .EQ. 4) Call RDINIT (15,0, 'MANAG4.DAT')
      IF (IMANAG .EQ. 5) Call RDINIT (15,0, 'MANAG5.DAT')
...
!    Calling the subroutine for relative death rate of leaves.
       CALL DEATHL(DAY,EMERG,DVS,DVSDLT,RDRTMP,RDRSHM,RDRL,
     $	 TRANRF,LAI,LAICR,WLVG,RDRNS,NNI,
...
!    organs (kg N ha-1 d-1)
       CALL NDEMND(NMAXLV,NMAXST,NMAXRT,NMAXSO,WLVG,WST,WRT,
     $   WSO,ANLV,ANST,ANRT,ANSO,TCNT,NDEML,NDEMS,
     $   NDEMR,NDEMSO)
...
!    Calling the subroutine for calculating N translocated from leaves, stem, and roots (kg N ha-1 d-1)
      IF (EMERG) THEN 
	 CALL NTRANS(RNSO,ATNLV,ATNST,ATNRT,ATN, RNTLV,RNTST,
     $    RNTRT)
	ELSE
	 RNTLV= 0.
	 RNTRT= 0.
...
!    N uptake rate (NUPTR) over the leaves, stem and roots (kg N ha-1 d-1)
       IF (EMERG) CALL RNUSUB(DAY,DAYEM,EMERG,NDEML,NDEMS,NDEMR,NUPTR,
...
* ----- Data for summary output and daily output
...
*-----  mean values for indexes NNI and TRANRF
...
*----- set crop variables to zero at TERMIN
        WLVG= 0.
	  WLVD= 0.
	  WST= 0.
	  WSO= 0.
